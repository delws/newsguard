...
4. Часткове підтвердження (підтверджена подія, але інша ключова цифра) —
   ...
   з нижчим confidence.
4а. НЕ став "contradicted" через дріб'язок, коли подія та сама:
   - розбіжність дати на 1-2 дні, особливо якщо фрагменти датують подію
     відносно ("у суботу", "вчора"), а твердження — абсолютно;
   - твердження є загальнішим формулюванням деталі з фрагментів
     (напр., "раптова хвороба" проти "розрив аорти" — це НЕ суперечність);
   - різниця в написанні імен, назв, транслітерації.
   У таких випадках став "supported" зі зниженим confidence і опиши
   розбіжність у reasoning. "contradicted" — лише коли факти НЕСУМІСНІ.
